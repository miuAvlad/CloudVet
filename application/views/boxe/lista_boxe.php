...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Lista boxe</h1>
    </div>
                                        ...
                                        <?php foreach ($boxa->caini as $caine) : ?>
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">
                                                <a href="<?php echo base_url() ?>/caini/editeaza_caine/<?= $caine->NrCrt ?>/0/NULL">
                                                    Intrare adapost:<?= $caine->DataIntrareAdapost ?> // CIP:<?= $caine->NrCip ?> // Varsta: <?php $diff = date_diff(date_create($caine->DataNastere), date_create(date("Y-m")));
                                                                                                                                                echo $diff->format('%y') . " ani " . $diff->format('%m') . " luni " ?>
                                                </a>
